...
export function Sidebar({
  ...
  return (
    ...
    >
      ...
      {/* As Definições vivem no menu da conta — ver `UserCard`. */}
      <UserCard collapsed={collapsed} showSettings={showSettings} />
      <TrialBadge
 ...
 */
function UserCard({ collapsed, showSettings }: { collapsed: boolean; showSettings: boolean }) {
  const { session } = useSession();
  const [open, setOpen] = useState(false);
    ...
    <div ref={ref} className="relative border-t border-line p-2">
      {open && (
        ...
        >
          ...
          {/*
            As Definições estão aqui e não no menu de navegação.

            Não são um destino do dia — ninguém "vai às definições" como vai aos
            atletas ou ao calendário; vai-se lá uma vez, ao montar a academia, e
            depois de mês a mês. No fundo da barra, como item solto, ocupavam
            uma linha permanente de um menu que já cresceu para vinte, e liam-se
            como mais uma área. Junto da conta é onde toda a gente as procura:
            é o menu de "coisas sobre mim e sobre este sítio".
          */}
          {showSettings && (
            <div className="mt-1 border-t border-line pt-1">
              <NavLink
                ...
                className={({ isActive }) =>
                  cx(
                    "flex w-full items-center gap-2.5 rounded-[6px] px-2.5 py-2 text-left transition-colors duration-[120ms] hover:bg-sunken",
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-body font-medium text-ink">{SETTINGS_ITEM.label}</span>
                  <span className="block truncate text-[11px] text-ink-3">Academia, cargos, época, pagamentos</span>
                </span>
              </NavLink>
            </div>
          )}
