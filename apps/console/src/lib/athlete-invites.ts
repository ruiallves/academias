...
export const inviteAthlete = (id: string) =>
  apiPost<{ ok: true; email: string }>(`/api/athletes/${encodeURIComponent(id)}/convite`, {});

/** A vários de uma vez — a acção em massa da lista de atletas. */
export const inviteAthletes = (ids: string[]) =>
  apiPost<{ ok: true; enviados: number; falhas: { id: string; reason: string }[] }>("/api/athletes/convites", { ids });
