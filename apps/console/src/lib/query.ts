...
export function useApi<T>(path: string | null, params?: Record<string, string | undefined>): Query<T> {
  const [data, setData] = useState<T>();
  const [loading, setLoading] = useState(path !== null);
  const [error, setError] = useState<string | null>(null);
  const [status, setStatus] = useState<number | null>(null);

  // Serializado para a identidade do objecto não disparar um pedido a cada render.
  ...
  const run = useCallback(() => {
    ...
    // `params` entra pela chave serializada — ver acima.
